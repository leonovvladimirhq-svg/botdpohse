...
>
> **Сделано 23.09.2026.** Внешняя точка доступа — посредник на Cloudflare Workers,
> **без аренды виртуальной машины**: `https://tg-relay-dpo.leonov-vladimir-hq.workers.dev`
> (код и инструкция — [`relay/`](./relay/)). Бот поднят и работает, в логе
> `getUpdates … 200 OK`. База, лог и обращения к Yandex AI Studio остаются в РФ.

Как это было сделано — и как повторить, если придётся разворачивать заново:
