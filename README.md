...
| ⏸️ **Telegram** — дополнительный | корень репозитория | Остановлена 2026-09-06. Для возврата — развернуть посредник из [`relay/`](./relay/) |
...
>
> Нужна **внешняя точка доступа к Bot API**. Решение — посредник на Cloudflare
> Workers, **без аренды виртуальной машины**: код и инструкция в
> [`relay/`](./relay/). Развернуть его нужно один раз (нужен бесплатный аккаунт
> Cloudflare), дальше бот работает как раньше. База, лог и обращения к Yandex AI
> Studio остаются в РФ.

Порядок восстановления:

```bash
# 1. Развернуть Worker по relay/README.md → получить адрес *.workers.dev
