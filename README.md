...
| ⏸️ **Telegram** — дополнительный | корень репозитория | Остановлена 2026-09-06. Возврат требует внешней точки доступа к Bot API — см. «Восстановление Telegram-версии» |
...
> ⚠️ **Просто запустить его обратно уже нельзя.** С сентября 2026 `api.telegram.org`
> недоступен с любых серверов в РФ. Замер 22.09.2026 на четырёх наших ВМ в разных
> каталогах и подсетях дал идентичный результат: `github.com` → 200, `api.telegram.org` →
> HTTP 000. **Фильтрация идёт по назначению, а не по нашему IP**, поэтому перебор адресов
> и аренда новых ВМ ради «рабочего IP» результата не дадут.
>
> Для работы Telegram-канала нужна **внешняя точка доступа к Bot API** —
> переменная `TELEGRAM_RELAY_URL`. Способ её реализации на 22.09.2026 не выбран
> (вариант с арендой ВМ в Казахстане заказчиком отклонён). База, лог и обращения
> к Yandex AI Studio при любом варианте остаются в РФ.

Порядок восстановления, когда точка доступа появится:

```bash
# На ВМ бота: обновить bot.py и docker-compose.yml из репозитория,
# в /opt/faqbot/.env добавить TELEGRAM_RELAY_URL=https://<хост>
ssh -i ~/.ssh/yc_faqbot_key yc-user@89.169.142.74
...
- Внешняя точка лежит — перезапускается только `faqbot`; MAX-бот работает независимо.
- Нельзя держать два поллера на одном токене одновременно — Telegram отдаст 409 Conflict.
- До запуска в эксплуатацию — юридическое заключение по ч. 8 ст. 10 149-ФЗ и уведомление
  РКН о трансграничной передаче (см. записку).
