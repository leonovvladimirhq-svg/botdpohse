...
| ⏸️ **Telegram** — дополнительный | корень репозитория | Остановлена 2026-09-06. Возврат — только через релей в Yandex Cloud kz1, см. [`relay/`](./relay/) |
...
├── relay/                 # Релей Bot API в Yandex Cloud kz1: install-relay.sh + README
├── .env.example
...
> недоступен с любых серверов в РФ — ТСПУ режет все подсети Telegram (проверено
> 21.09.2026: и старый рабочий пин `149.154.167.220`, и остальные IP — таймаут).
> Нужен **релей за пределами РФ**: reverse-proxy в Yandex Cloud kz1 (Казахстан).
> Как его купить, поставить и подключить — [`relay/README.md`](./relay/README.md),
> там же юридический контекст из аналитической записки (152-ФЗ, 149-ФЗ, уведомление РКН).

Порядок восстановления:

```bash
# 1. Релей поднят по relay/README.md (ВМ в kz1 покупает владелец облака)
# 2. На ВМ бота: обновить bot.py и docker-compose.yml из репозитория, в .env добавить
#    TELEGRAM_RELAY_URL=https://<хост релея>
ssh -i ~/.ssh/yc_faqbot_key yc-user@89.169.142.74
