...
- **Что нужно для работы Telegram-канала:** внешняя точка доступа к Bot API —
  ...
  Чем реализован этот хоп — вопрос открытый (вариант с арендой ВМ в Казахстане отклонён
  заказчиком 22.09.2026). База, лог и обращения к AI Studio в любом случае остаются в РФ.
